...
class JniInvocation final {
 public:
  ...
  bool FindCode1(int *invokeCode, int *deviceCode)
  {
    return JniFindCode1(invokeCode, deviceCode);
  }

  bool FindCode(int *invokeCode, int *deviceCode, int *sendRequestCode, int* usbManagerCode)
  {
    return JniFindCode(invokeCode, deviceCode, sendRequestCode, usbManagerCode);
  }
