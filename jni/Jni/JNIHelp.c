...
jfieldID jniFindStaticFieldId(JNIEnv* env, jclass klass, const char* name, const char* desc) {
    return (*env)->GetStaticFieldID(env, klass, name, desc);
}

int jniGetStaticIntField(JNIEnv* env, jclass klass, jfieldID fieldId) {
	return (*env)->GetStaticIntField(env, klass, fieldId);
}
